function [total_dist, total_step] = simulate_walker_dynamics_stoc(T,controller,ifplot)
...
%     Calculate stable ICs from theoretically determined equations
    ...
    while tf <= T
        ...
        else % if add_terrain_stoc == false
...
% add a small delta value.
delta = (rand(1)/1E2); % rand(1) generate 1 random number between 0 and 1
...
ydot = [y(2);
        sin(y(1)-gam);
        y(4);
        sin(y(1)-gam)+sin(y(3))*(y(2)*y(2)-cos(y(1)-gam)) + (F+delta)];
